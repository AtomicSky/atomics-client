...
public final class BorderConfig {
    ...
    public double clampX(double x) {
        return Math.max(this.minX + 0.001, Math.min(this.maxX - 0.001, x));
    }

    public double clampZ(double z) {
        return Math.max(this.minZ + 0.001, Math.min(this.maxZ - 0.001, z));
    }
    ...
    public double clampToSide(double x, double z, boolean wantPlainsSide, boolean isX) {
        ...
        if (wantPositiveSide) {
            return Math.max(value, this.dividerCoordinate + 0.001);
        }
        return Math.min(value, this.dividerCoordinate - 0.001);
    }
