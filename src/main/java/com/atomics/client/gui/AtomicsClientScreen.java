...
public class AtomicsClientScreen extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        context.centeredText(this.font, Component.literal("Atomics Client"), this.width / 2, 14, TEXT_MAIN);
        context.centeredText(this.font, Component.literal(selectedTab.label + " Settings"), this.width / 2, 28, TEXT_MUTED);
        ...
        for (DrawLabel label : labels) {
            ...
            if (label.lineWidth > 0) {
                ...
                context.centeredText(this.font, Component.literal(label.text), label.x, label.y, textColor(label.color));
            } else {
                context.text(this.font, Component.literal(label.text), label.x, label.y, textColor(label.color));
            }
        }
    ...
    private void renderStatus(GuiGraphicsExtractor context) {
        ...
        if (text.isBlank()) {
            context.text(this.font, Component.literal("Changes are live. Save to keep them after restart."), OUTER_MARGIN, this.height - 14, TEXT_MUTED);
            return;
        }
        ...
        context.text(this.font, status, x + 8, y + 5, TEXT_MAIN);
    }

    private void renderPreviewPanel(GuiGraphicsExtractor context, int mouseX, int mouseY) {
        int x = previewX;
        int y = contentTop;
        int w = previewWidth;
        context.text(this.font, Component.literal(selectedTab.label + " Preview"), x, y - 2, textColor(0xFFFFFF));
        int boxTop = y + 22;
        int boxBottom = contentBottom - 12;
        context.fill(x, boxTop, x + w, boxBottom, 0xAA202020);
        context.outline(x, boxTop, w, boxBottom - boxTop, 0x90FFFFFF);
        context.centeredText(this.font, Component.literal(selectedTab.label + " Preview"), x + w / 2, boxTop + 14, textColor(0xFFFFFF));
        ...
        if (client.player != null && selectedTab == Tab.TOTEM) {
            ...
            InventoryScreen.extractEntityInInventoryFollowsMouse(context, entityLeft, entityTop, entityRight, entityBottom, entitySize, 0.06f, mouseX, mouseY, client.player);
            ...
            context.centeredText(this.font, Component.literal("Preview will appear here"), x + w / 2, boxTop + (boxBottom - boxTop) / 2, textColor(0xAAAAAA));
        }
    }

    private void renderStatsDashboard(GuiGraphicsExtractor context) {
        int x = leftX;
        ...
        context.text(this.font, Component.literal("Stats"), x, y - 2, textColor(0xFFFFFF));
        y += 20;
        ...
        if (y >= contentTop && y <= contentBottom - 10) {
            context.text(this.font, Component.literal("Line Graph Fields"), x, y + 4, textColor(0xFFFFFF));
        }
        y += 18;
    ...
    private int renderCounterPanel(GuiGraphicsExtractor context, String title, PvpStatsManager.Counters counters, int x, int y, int width) {
        return renderCounterPanel(context, title, counters, x, y, width, false);
    }

    private int renderCounterPanel(GuiGraphicsExtractor context, String title, PvpStatsManager.Counters counters, int x, int y, int width, boolean signed) {
    ...
    private int renderCounterGraphPanel(GuiGraphicsExtractor context, String title, PvpStatsManager.Counters counters, int x, int y, int width) {
        ...
        if (y + padding >= contentTop && y + padding <= bottom) {
            context.text(this.font, Component.literal(title), x + padding, y + padding, textColor(0xFFFFFF));
        }
        ...
        for (int i = 0; i < bars.length; i++) {
            ...
            context.text(this.font, Component.literal(bar.label), x + padding, rowY, textColor(0xD8D8D8));
            context.fill(barX, rowY + 2, barX + barW, rowY + 11, 0x66000000);
            context.fill(barX, rowY + 2, barX + fillW, rowY + 11, textColor(bar.color));
            context.text(this.font, Component.literal(formatGraphValue(bar.value)), barX + barW + 6, rowY, textColor(0xD8D8D8));
        }
    ...
    private int renderSessionLineGraphPanel(GuiGraphicsExtractor context, String title, List<PvpStatsManager.SessionSnapshot> history, int x, int y, int width) {
        ...
        if (y + padding >= contentTop && y + padding <= bottom) {
            ...
            context.text(this.font, Component.literal(title + " - " + count), x + padding, y + padding, textColor(0xFFFFFF));
        }
        ...
        if (history.isEmpty() || activeCount == 0) {
            ...
            if (messageY >= contentTop && messageY <= bottom) {
                context.centeredText(this.font, Component.literal("No selected session data yet"), graphX + graphW / 2, messageY, textColor(0xAAAAAA));
            }
        } else {
            float max = 0.0f;
            for (HistoryMetric metric : metrics) {
        ...
        for (HistoryMetric metric : metrics) {
            ...
            if (legendY >= contentTop && legendY <= bottom) {
                ...
                context.text(this.font, Component.literal(metric.label), legendX + 13, legendY, textColor(0xD8D8D8));
            }
            legendX += labelW + 8;
        }
    ...
    private void drawGraphLine(GuiGraphicsExtractor context, int x1, int y1, int x2, int y2, int color) {
    ...
    private void fillContent(GuiGraphicsExtractor context, int x1, int y1, int x2, int y2, int color) {
    ...
    private int renderTextPanel(GuiGraphicsExtractor context, String title, String[] lines, int x, int y, int width) {
        int padding = 8;
        int lineHeight = 13;
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (textY >= contentTop && textY <= bottom) {
                ...
                context.text(this.font, Component.literal(line.text), x + padding, textY, textColor(line.color));
            }
        }
        return y + height;
    }

    private void drawClippedPanelBorder(GuiGraphicsExtractor context, int x, int y, int width, int height, int visibleTop, int visibleBottom, int color) {
    ...
    private void renderTotemSizePreview(GuiGraphicsExtractor context, ItemStack stack, String label, int centerX, int centerY, float scale) {
        String display = label + "  " + formatDecimal(scale, 2) + "x";
        context.centeredText(this.font, Component.literal(display), centerX, centerY - 22, textColor(0xFFFFFF));
    ...
    private void renderScaledItem(GuiGraphicsExtractor context, ItemStack stack, int centerX, int centerY, float scale) {
    ...
    private class TabButtonWidget extends AbstractWidget {
        ...
        protected void extractWidgetRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
            ...
            context.centeredText(font, Component.literal(label), x + width / 2, y + 8, selected ? TEXT_MAIN : TEXT_MUTED);
        }
    ...
    private class SectionHeaderWidget extends AbstractWidget {
        ...
        protected void extractWidgetRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
            ...
            context.centeredText(font, Component.literal(marker), x + 15, y + 7, collapsed ? TEXT_MUTED : TEXT_MAIN);
            context.text(font, Component.literal(title), x + 28, y + 7, TEXT_MAIN);
            context.text(font, Component.literal(collapsed ? "collapsed" : "expanded"), x + width - font.width(collapsed ? "collapsed" : "expanded") - 10, y + 7, TEXT_MUTED);
        }
    ...
    private class DropdownButtonWidget extends AbstractWidget {
        ...
        protected void extractWidgetRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
            ...
            context.text(font, Component.literal(label), x + 8, y + 7, TEXT_MAIN);
            context.text(font, Component.literal(right), x + width - font.width(right) - 8, y + 7, open ? ACCENT : TEXT_MUTED);
        }
