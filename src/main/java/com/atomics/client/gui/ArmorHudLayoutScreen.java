...
class ArmorHudLayoutScreen extends Screen {
    ...
    private void renderHudPreview(GuiGraphicsExtractor context) {
        int left = hudX - 4;
        int top = hudY - 4;
        ...
        for (int i = 0; i < stacks.length; i++) {
            ...
            context.item(stacks[i], itemX, itemY);
            ...
            if (!text.isEmpty()) {
                ...
                context.text(this.font, text, textX, textY, 0xFF55FF55);
    ...
    private void renderPreviewSlot(GuiGraphicsExtractor context, int x, int y) {
        context.blit(RenderPipelines.GUI_TEXTURED, HOTBAR_SPRITE_TEXTURE, x, y, 0.0f, 0.0f, 22, 22, 182, 22);
    }

    private void renderVanillaHudReference(GuiGraphicsExtractor context) {
        ...
        context.item(new ItemStack(Items.SHIELD), offhandX + 3, offhandY + 3);
    ...
    private void renderGuiIconRow(GuiGraphicsExtractor context, int x, int y, int count, Identifier background, Identifier foreground) {
    ...
    private void renderItemDurabilityBar(GuiGraphicsExtractor context, int x, int y, int width, int index) {
