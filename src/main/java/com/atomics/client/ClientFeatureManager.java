...
public final class ClientFeatureManager {
    ...
    public static void tick(Minecraft client) {
        ...
        if (cfg.visual.timeChangerEnabled) {
            client.level.setTimeFromServer(client.level.getGameTime(), cfg.visual.timeOfDay, false);
    ...
    public static void renderHud(GuiGraphics context) {
        TpsConfig cfg = liveConfig();
    ...
    public static boolean onZoomScroll(double verticalAmount) {
        ...
        if (client.player != null) {
            ...
            if (now - lastZoomFeedbackMillis > 70L) {
                client.player.displayClientMessage(Component.literal("Zoom: " + String.format(Locale.US, "%.2fx", cfg.visual.zoomMultiplier)).withStyle(ChatFormatting.AQUA), true);
    ...
    public static void runMacro(Minecraft client, int index) {
        ...
        if (message == null || message.isBlank()) {
            if (client.player != null) {
                client.player.displayClientMessage(Component.literal("Macro " + (index + 1) + " is empty").withStyle(ChatFormatting.YELLOW), true);
            }
            return;
        }
    ...
    private static void renderReachDisplay(GuiGraphics context, Minecraft client) {
        ...
        context.drawString(client.font, lastReachText, Math.round(x / scale), Math.round(y / scale), color);
        context.pose().popMatrix();
    }

    private static void renderArmorHudAndWarnings(GuiGraphics context, Minecraft client, TpsConfig.VisualSettings visual) {
    ...
    private static void renderArmorHud(GuiGraphics context, Minecraft client, ArmorPiece[] armor, TpsConfig.VisualSettings visual) {
        int visible = 0;
        ...
        for (ArmorPiece piece : armor) {
            ...
            context.renderItem(piece.stack(), itemX, itemY);
            context.renderItemDecorations(client.font, piece.stack(), itemX, itemY);
            ...
            if (!durability.isEmpty()) {
                ...
                context.pose().pushMatrix();
                context.pose().scale(scale, scale);
                context.drawString(client.font, durability, textX, textY, color);
                context.pose().popMatrix();
            }
            if (vertical) {
                y += spacing;
    ...
    private static void renderArmorHudSlot(GuiGraphics context, int x, int y, int visible) {
    ...
    private static void warnLowArmorDurability(Minecraft client, ArmorPiece[] armor, int warningPercent) {
        ...
        if (!key.equals(lastArmorWarningKey) || now - lastArmorWarningMillis > 2500L) {
            client.player.displayClientMessage(Component.literal("Low armor durability: " + lowest.slotName() + " " + lowest.remaining() + "/" + lowest.maxDamage())
    ...
    private static void disableTimeChangerIfNeeded(Minecraft client) {
        ...
        client.level.setTimeFromServer(client.level.getGameTime(), client.level.getDayTime(), true);
